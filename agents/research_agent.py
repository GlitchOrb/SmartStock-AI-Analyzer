...
from schemas.agents import ResearchAgentOutput, TimelineEvent
...
def run_research_agent(
    ...
    1. Fetch news via rag/loader.py
    2. Build FAISS vectorstore via rag/vectorstore.py
    3. Retrieve top-5 chunks
    4. Call Gemini with chunks as context
    5. Return ResearchAgentOutput

    If 0 articles found, returns output with data_quality="insufficient"
    and warnings set.
    """
    ...
    # 1. Fetch news
    log_agent("ResearchAgent", f"뉴스 수집 시작: {ticker}")
    try:
        documents, loader_warnings = load_rss_news(ticker)
        all_warnings.extend(loader_warnings)
    except Exception as e:
        log_agent("ResearchAgent", f"[red]뉴스 수집 실패: {e}[/red]")
    ...
    if news_count == 0:
        log_agent("ResearchAgent", "[yellow]뉴스 0건 → RAG 건너뜀, 기본값 반환[/yellow]")
    ...
    # 2. Build vectorstore
    log_agent("ResearchAgent", f"벡터스토어 구축 중 ({news_count} 문서)...")
    try:
        vs = build_vectorstore(ticker, documents)
    except Exception as e:
        log_agent("ResearchAgent", f"[red]벡터스토어 구축 실패: {e}[/red]")
    ...
    if vs is not None:
        ...
        log_agent("ResearchAgent", "[yellow]벡터스토어 없음 → 원본 문서 사용[/yellow]")
    ...
    # Attempt #1
    try:
        log_agent("ResearchAgent", f"Gemini 호출 중 ({depth.value} 모드)...")
        response = gemini_client.invoke("ResearchAgent", _SYSTEM_PROMPT, user_prompt, depth)
        parsed = _safe_json_loads(response)
    except Exception as e:
        log_agent("ResearchAgent", f"[red]Gemini 호출 실패: {e}[/red]")
        all_warnings.append(f"AI 분석 실패: {e}")

    # If parsing failed, retry once
    if not parsed or "key_themes" not in parsed:
        log_agent("ResearchAgent", "[yellow]JSON 파싱 실패 → 재시도 중...[/yellow]")
        try:
            retry_prompt = (
                f"{user_prompt}\n\n"
            ...
            response2 = gemini_client.invoke("ResearchAgent", _RETRY_SYSTEM_PROMPT, retry_prompt, depth)
            parsed = _safe_json_loads(response2)
        except Exception as e2:
            log_agent("ResearchAgent", f"[red]재시도 실패: {e2}[/red]")
            all_warnings.append(f"AI 재시도 실패: {e2}")

    # If still failed, use safe defaults
    if not parsed or "key_themes" not in parsed:
        log_agent("ResearchAgent", "[red]안전 기본값 사용[/red]")
